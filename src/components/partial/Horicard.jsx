...
function Horicard({ data }) {
    ...
    console.log(data);
    return (
        <div className='container mx-auto px-5 scrollable-element bg-transparent'>
            <div className="flex flex-no-wrap overflow-x-auto">
                {data.map((d, i) => (
                    <Link 
                        ...
                        key={i} 
                        className="flex-none w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5 p-4 bg-transparent"
                    >
                      
                        
                        <div className="bg-zinc-900 rounded shadow-lg overflow-hidden bg-transparent" style={{ height: 'auto' }}>
                            <img
                            ...
                            />
                            <div className="p-4">
                                <h1 className='text-xl md:text-2xl font-black mb-3 bg-transparent'>{(d.original_title || d.name || d.title || d.original_name).slice(0, 30)}</h1>
                            </div>
                        </div>
                    </Link>
                ))}
